...
func_exit:
    ...
    X_ASSERT(currentStrategyID >= 0
             && currentStrategyID < STRATEGY_NB);              // Vérifie que l'ID de la stratégie actuelle est valide
    X_ASSERT(strategyWrapper[currentStrategyID].name != NULL); // Vérifie que le nom de la stratégie actuelle n'est pas NULL
    X_ASSERT(strategyWrapper[currentStrategyID].prepare
             != NULL); // Vérifie que la fonction de préparation de la stratégie actuelle n'est pas NULL
    X_ASSERT(strategyWrapper[currentStrategyID].execute
             != NULL); // Vérifie que la fonction d'exécution de la stratégie actuelle n'est pas NULL
...
void strategy_wrapper__execute(seq_t *way, point_t *initial, point_t *final)
{
