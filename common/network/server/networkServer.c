                         ...
                         uint32_t p_ulPayloadSize) 
{
    ...
    uint32_t l_ulTotalSize = 5 + p_ulPayloadSize; // 4 bytes for size + 1 byte for type + payload
    ...
    {
        memcpy(l_pucBuffer + 5, p_pvPayload, p_ulPayloadSize);
    }
                               ...
                               uint8_t* p_ptucMsgType) 
{
    ...
    // extract the payload size (network order)
