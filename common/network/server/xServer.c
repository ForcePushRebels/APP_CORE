...
int xServerConfigure(const ServerConfig *p_ptConfig)
{
    ...
    X_LOG_INFO(
        "Server configured for port %d (TLS: %s)", p_ptConfig->t_usPort, p_ptConfig->t_bUseTls ? "enabled" : "disabled");
    return SERVER_OK;
}
...
int xServerStart(void)
{
    ...
    if (epoll_ctl(s_tServer.t_iEpollFd, EPOLL_CTL_ADD, s_tServer.t_ptListenSocket->t_iSocketFd, &l_tEvent) < 0)
    {
...
int xServerStop(void)
{
    ...
    if (s_tServer.t_ptListenSocket != NULL)
    {
        epoll_ctl(s_tServer.t_iEpollFd, EPOLL_CTL_DEL, s_tServer.t_ptListenSocket->t_iSocketFd, NULL);
        networkCloseSocket(s_tServer.t_ptListenSocket);
        s_tServer.t_ptListenSocket = NULL;
    }
...
static void *serverMainThread(void *p_pvArg)
{
    ...
    {
        int l_iNumEvents = epoll_wait(l_ptServer->t_iEpollFd, l_atEvents, SERVER_MAX_CLIENTS + 1, SERVER_EPOLL_TIMEOUT);
...
static int handleNewConnection(NetworkSocket *p_ptClientSocket, const NetworkAddress *p_ptClientAddr)
{
    ...
    snprintf(l_ptClient->t_acClientName,
             sizeof(l_ptClient->t_acClientName),
             "%s:%d",
             p_ptClientAddr->t_cAddress,
             p_ptClientAddr->t_usPort);

    // Setup TLS if enabled
    if (s_tServer.t_sConfig.t_bUseTls && s_tServer.t_ptTlsEngine != NULL)
    {
        int l_iTlsResult
            = tlsEngineAttachSocket(s_tServer.t_ptTlsEngine, p_ptClientSocket->t_iSocketFd, &l_ptClient->t_ptTlsSession);
        if (l_iTlsResult != CERT_OK)
        {
    ...
    snprintf(l_ptClient->t_tClientTask.t_acTaskName,
             sizeof(l_ptClient->t_tClientTask.t_acTaskName),
             "client_%u",
             l_ptClient->t_tId);
...
static int clientReceiveMessage(clientCtx *p_ptClient)
{
    ...
    if (l_ulPayloadSize > 1)
    {
        ...
        {
            // TLS receive
            l_iPayloadReceived = wolfSSL_read(
                p_ptClient->t_ptTlsSession, p_ptClient->t_aucRecvBuffer + PROTOCOL_HEADER_SIZE, (int)l_ulRemainingBytes);
        ...
        {
            // Plain TCP receive
            l_iPayloadReceived = networkReceive(
                p_ptClient->t_ptSocket, p_ptClient->t_aucRecvBuffer + PROTOCOL_HEADER_SIZE, l_ulRemainingBytes);
        }
        ...
        if ((uint32_t)l_iPayloadReceived != l_ulRemainingBytes)
        {
            ...
            {
                X_LOG_INFO("Client %u disconnected (payload read: %d/%u)",
                           p_ptClient->t_tId,
                           l_iPayloadReceived,
                           l_ulRemainingBytes);
                // IMPORTANT: Mark client as disconnected to stop the thread loop
    ...
    {
        X_LOG_WARN("Failed to parse frame from client %u: %s", p_ptClient->t_tId, protocolGetErrorString(l_iParseResult));
    }
...
bool xServerGetClientInfo(ClientID p_tClientId, char *p_ptcBuffer, int p_iBufferSize)
{
    ...
    snprintf(p_ptcBuffer,
             (size_t)p_iBufferSize,
