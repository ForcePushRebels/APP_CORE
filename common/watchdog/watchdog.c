...
void *watchdog_thread(void *arg)
{
    ...
    while (l_iStopFlag == OS_TASK_SECURE_FLAG && !l_iTerminate)
    {
        ...
        {
            X_LOG_TRACE(" Watchdog ping failed in thread: %s (code: 0x%x)", watchdog_get_error_string(l_iRet), l_iRet);
        }
...
static void timer_handler(union sigval sv)
{
    ...
    {
        X_LOG_TRACE(" WATCHDOG TIMEOUT - REAL TIME ERROR");
        exit(EXIT_FAILURE);
    }
}
...
int watchdog_init(int timeout_ms)
{
    ...
    {
        X_LOG_TRACE(" Failed to initialize watchdog task: %s (code: 0x%lx)", osTaskGetErrorString(l_ulReturn), l_ulReturn);
    ...
    if (l_ulReturn != OS_TASK_SUCCESS)
    {
        X_LOG_TRACE(" Failed to create watchdog thread: %s (code: 0x%lx)", osTaskGetErrorString(l_ulReturn), l_ulReturn);
        cleanupWatchdogResources();
    ...
    X_LOG_TRACE(" POSIX timer watchdog initialized (timeout=%ums)", l_ulActualTimeout);
...
{
    // atomic store of callback pointer
...
const char *watchdog_get_error_string(int error_code)
{
    switch (error_code)
    {
        case WATCHDOG_SUCCESS:
            return "Success";
        case WATCHDOG_ERROR_INIT_FAILED:
            return "Watchdog initialization failed";
        case WATCHDOG_ERROR_ALREADY_INIT:
            return "Watchdog already initialized";
        case WATCHDOG_ERROR_NOT_INIT:
            return "Watchdog not initialized";
        case WATCHDOG_ERROR_TIMER_CREATE:
            return "Failed to create watchdog timer";
        case WATCHDOG_ERROR_TIMER_SET:
            return "Failed to set/start watchdog timer";
        case WATCHDOG_ERROR_THREAD_CREATE:
            return "Failed to create watchdog thread";
        case WATCHDOG_ERROR_THREAD_FAILED:
            return "Watchdog thread operation failed";
        case WATCHDOG_ERROR_INVALID_PARAM:
            return "Invalid parameter for watchdog";
        default:
...
static int validateTimeout(int p_iTimeout)
{
    ...
    if (p_iTimeout > 0 && (p_iTimeout < WATCHDOG_MIN_TIMEOUT_MS || p_iTimeout > WATCHDOG_MAX_TIMEOUT_MS))
    {
        return WATCHDOG_ERROR_INVALID_PARAM;
    }
