...
int osSemWaitTimeout(t_OSSemCtx *p_pttOSSem, unsigned long p_ulTimeoutMs)
{
    ...
    if (p_pttOSSem->is_named)
    {
        l_iReturn = sem_timedwait(p_pttOSSem->sem_handle, &ts);
    }
    else
    {
        l_iReturn = sem_timedwait(&p_pttOSSem->sem, &ts);
    }
...
int osSemTryWait(t_OSSemCtx *p_pttOSSem)
{
    ...
    if (p_pttOSSem->is_named)
    {
        l_iReturn = sem_trywait(p_pttOSSem->sem_handle);
    }
    else
    {
        l_iReturn = sem_trywait(&p_pttOSSem->sem);
    }
