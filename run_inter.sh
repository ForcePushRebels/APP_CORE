...
if [ -n "$TARGET_MRPIZ" ]; then
    EXECUTABLE="build/bin/inter_mrpiz"
    ...
    print_warning "Use: scp ${EXECUTABLE} pi@<MRPIZ_ADDRESS>:inter_mrpiz"
    exit 0
else
    EXECUTABLE="build/bin/inter_intox"
fi
...
# Run the application
print_section "RUNNING INTER APPLICATION"
