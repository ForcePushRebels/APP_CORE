...
public class LengthPrefixedBinaryTCPClientImpl extends TCPClientDecorator {
    ...
    private final String tcpIgnorePrefix = JMeterUtils.getPropDefault("tcp.binary.ignore.prefix", null); 
    ...
    public String read(InputStream is, SampleResult sampleResult) throws ReadException{
        ...
        try {
		    ...
		    buffer = JOrphanUtils.baToHexString(msg);
		    if(log.isDebugEnabled()) {
		        log.debug("Read: " + msgLen + "\n" + buffer);
		    }
		    
		    if (tcpIgnorePrefix != null) {
		    	readOk = !buffer.matches(tcpIgnorePrefix);
		    } else {
		    	readOk = true;
		    }
            }
	...
	// in order to support expected no server response
